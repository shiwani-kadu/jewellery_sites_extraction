...
def fetch_product_data(link, token, cookies, materials_data, region):
    ...
    for attempt in range(max_retries):
        try:
            ...
            file_path = f'../../pages/{datetime.datetime.today().strftime("%Y%m%d")}/van_cleef_arpels_clips_brooch/{region}/{page_hash}.html'
...
def validate_input_files(region):
    ...
    """
    links_file = f'../../input_files/listings/van_cleef_arpels_clips_brooch_links_{region}.json'
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Scrape product data from celine_men_fine_jewellery.")
    ...
    try:
        output_filename = f'../../output_files/van_cleef_arpels_clips_brooch/{datetime.datetime.today().strftime("%Y%m%d")}'
        os.makedirs(output_filename,exist_ok=True)
        file_name= f'van_cleef_arpels_clips_brooch_product_{args.region}_{datetime.datetime.today().strftime("%Y%m%d")}'
