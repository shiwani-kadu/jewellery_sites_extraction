...
def load_cookies(region):
    ...
    path ('../../configs/cookies/repossi.json'), then retrieves cookies
...
def parse_links(response, data_list, b_url):
    ...
    try:
        ...
        links = html_content.xpath('//div[@class="product-card  relative "]/a[@class="flex flex-col"]/@href')
        # links = [b_url + link for link in links]
        links = [urljoin(b_url,link) for link in links]
        data_list.extend(links)
        # return bool(html_content.xpath('//a[@rel="next"]/@href')[0])
        return html_content.xpath('//a[@rel="next"]/@href')
    except Exception as e:
...
def process_url(base_url, token, cookies, headers, region):
    ...
    while True:
        ...
        logging.info(f"{base_url}: {response.status_code}")

        if not response:
            break

        next_page = parse_links(response, data_list, base_url)
        if not next_page:
            break
        base_url = urljoin(base_url,next_page[0])
        page += 1
...
def main():
    """
    Main function for scraping product links from Repossi's website based on region. This function
    ...
    """
    parser = argparse.ArgumentParser(description="Scrape product links from Repossi.")
    ...
    try:
        ...
        # cookies = load_cookies('hk')
    except Exception:
        return

    # Read input URLs from JSON file
    try:
        with open(f'../../input_files/repossi_categories.json', 'r') as f:
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        futures = [
            ...
            # executor.submit(process_url, url, token, cookies, HEADERS, 'hk')
            for url in input_urls[args.region]
            # for url in input_urls['hk']
        ]
    ...
    try:
        output_file = f'../../input_files/listing/repossi_links_{args.region}.json'
