...
def fetch_product_data(link, token, cookies, materials_data, region):
    ...
    for attempt in range(max_retries):
        try:
            ...
            os.makedirs(f'../../pages/{datetime.datetime.today().strftime("%Y%m%d")}/van_cleef_arpels_rings/{region}',exist_ok=True)
            file_path = f'../../pages/20250328/van_cleef_arpels_rings/{region}/{page_hash}.html'
            ...
            with open(f'../../pages/{datetime.datetime.today().strftime("%Y%m%d")}/van_cleef_arpels_rings/{region}/{page_hash}.html', 'w',encoding='utf-8') as f:
...
def validate_input_files(region):
    ...
    """
    links_file = f'../../input_files/listings/van_cleef_arpels_rings_links_{region}.json'
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Scrape product data from van cleef and arpels rings.")
    ...
    try:
        output_filename = f'../../output_files/van_cleef_arpels_rings/{datetime.datetime.today().strftime("%Y%m%d")}'
        os.makedirs(output_filename,exist_ok=True)
        file_name= f'van_cleef_arpels_rings_product_{args.region}_{datetime.datetime.today().strftime("%Y%m%d")}'
