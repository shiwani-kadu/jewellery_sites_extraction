...
def fetch_product_data(link, token, cookies, materials_data, region):
    ...
    for attempt in range(max_retries):
        try:
            ...
            os.makedirs(f'../../pages/{datetime.datetime.today().strftime("%Y%m%d")}/van_cleef_arpels_earrings/{region}',exist_ok=True)
            file_path = f'../../pages/{datetime.datetime.today().strftime("%Y%m%d")}/van_cleef_arpels_earrings/{region}/{page_hash}.html'
            ...
            with open(f'../../pages/{datetime.datetime.today().strftime("%Y%m%d")}/van_cleef_arpels_earrings/{region}/{page_hash}.html', 'w',encoding='utf-8') as f:
...
def validate_input_files(region):
    ...
    """
    links_file = f'../../input_files/listings/van_cleef_arpels_earrings_links_{region}.json'
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Scrape product data from van cleef and arpels earrings.")
    ...
    try:
        output_filename = f'../../output_files/van_cleef_arpels_earrings/{datetime.datetime.today().strftime("%Y%m%d")}'
        os.makedirs(output_filename,exist_ok=True)
        file_name= f'van_cleef_arpels_earrings_product_{args.region}_{datetime.datetime.today().strftime("%Y%m%d")}'
