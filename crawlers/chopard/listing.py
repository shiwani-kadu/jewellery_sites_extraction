...
def load_cookies(region):
    ...
    path ('../../configs/cookies/chopard.json'), then retrieves cookies
...
def parse_links(response, data_list, b_url):
    ...
    try:
        ...
        links = []
        # if not html_content.xpath('//button[@class="btn btn--primary btn-outline-primary more link-standard"]'):
        links = html_content.xpath('//li[@class="product-grid__product product"]//a[@class="product"]/@href')
        links = [urljoin(b_url, link) for link in links]
        # data_list.extend(links)
        return bool(html_content.xpath('//button[@class="btn btn--primary btn-outline-primary more link-standard"]')), links
    except Exception as e:
...
def process_url(base_url, token, cookies, headers, region):
    ...
    item = 28
    while True:
        target_url = f"{base_url}?start={start}&sz={item}&p={page}"
        # api_url = f"http://api.scrape.do?token={token}&url={target_url}"
        impersonate_version = random.choice(BROWSER_VERSIONS)
        response = fetch_page(target_url, cookies, headers, impersonate_version)
        logging.info(f"{target_url}: {response.status_code}")

        if not response:
            break

        next_page,data_list = parse_links(response, data_list, base_url)
        if not next_page:
            break
...
def main():
    """
    Main function for scraping product links from Chopard's website based on region. This function
    ...
    """
    parser = argparse.ArgumentParser(description="Scrape product links from Chopard.")
    ...
    try:
        with open(f'../../input_files/chopard_categories.json', 'r') as f:
    ...
    try:
        output_file = f'../../input_files/listing/chopard_links_{args.region}.json'
