...
def validate_input_files(region):
    ...
    """
    links_file = f'../../input_files/listing/celine_men_fine_jewelry_links_{region}.json'
...
def load_cookies(region):
    ...
    """
    try:
        with open('../../configs/cookies/celine_men_fine_jewelry.json', 'r') as f:
            cookies = json.load(f)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Scrape product data from celine_men_fine_jewelry.")
    ...
    try:
        ...
        file_name= f'celine_men_fine_jewelry_product_{args.region}_{datetime.datetime.today().strftime("%Y%m%d")}'
        uniq_data_list = []
        for d in data_list:
