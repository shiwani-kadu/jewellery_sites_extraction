...
def parse_links(response, data_list, b_url):
    ...
    try:
        ...
        links = html_content.xpath('//a[@class="product__name u-expand u-inline-block u-blue"]/@href').getall()
        links = [link if 'https://' in link else b_url + link for link in links]
        data_list.extend(links)
        next_page = html_content.xpath("//a[@class='pages-item-next']/@href").get()
        if next_page:
            return bool(next_page)
        else:
            return bool(html_content.xpath("//a[@class='page last']/@href").get())
    except Exception as e:
...
def process_url(base_url, token, cookies, headers, region):
    ...
    while True:
        ...
        target_url = urllib.parse.quote(f"{base_url}?p={page}")
...
def main():
    ...
    try:
        with open(f'../../input_files/chaumet_categories.json', 'r') as f:
    ...
    try:
        output_file = f'../../input_files/listing/chaumet_links_{args.region}.json'
